...
fn replay_real_backbone_capture() {
    ...
    for rec in bytes[start..].chunks_exact(24) {
        ...
        let payload: Vec<u8>;
        // The assembler owns the table now, so ask it: a hand-copied list here would
        // drift from the one it actually filters on.
        let slice: &[u8] = if assembler.handles(pgn) {
            fp_frames += 1;
            if data.len() < 8 {
                continue;
            }
