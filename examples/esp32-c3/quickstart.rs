//! # Quickstart ESP32-C3 Example
//!
//! Complete ESP32-C3 (RISC-V) example using Embassy and esp-hal.
//!
//! ## Compilation
//! ```bash
...
//!
//! # Compiler
//! cargo build --example esp32c3_quickstart --target riscv32imc-unknown-none-elf --features embedded-examples
//!
//! # Flash
//! cargo run --example esp32c3_quickstart --target riscv32imc-unknown-none-elf --features embedded-examples
//! ```
//!
//! ## Required hardware
//! - ESP32-C3 DevKit
//! - CAN transceiver (e.g. SN65HVD230)
//! - Connections (adjust for your board):
//!   - GPIO4 → CAN TX
//!   - GPIO5 → CAN RX
//!   - GPIO8 → LED (optional)
...
// ============================================================================
// CanBus implementation for ESP32-C3
// ============================================================================
...
// ============================================================================
// Timer implementation for ESP32-C3
// ============================================================================
...
async fn main(spawner: Spawner) {
    esp_println::println!("=== ESP32-C3 (RISC-V) NMEA2000 Quickstart ===");

    // 1. Initialize ESP32-C3
    ...
    // 3. Configure LED (GPIO8 on ESP32-C3-DevKitM-1)
    ...
    // Note: adjust pins to match your hardware wiring
    ...
    let iso_name = IsoName::builder()
        .unique_number(54321) // Unique serial number (different from ESP32-S3)
    ...
    let manager = match AddressManager::new(can_bus, timer, iso_name.raw(), 43).await {
        Ok(mgr) => {
    ...
    // 7. Share the manager across tasks via a mutex
    let manager_mutex = MANAGER_CELL.init(Mutex::new(manager));

    // 8. Spawn concurrent tasks
...
async fn task_send_position(manager: &'static Mutex<CriticalSectionRawMutex, AddressManagerType>) {
    ...
    loop {
        ...
        position.latitude = 48.8566; // Latitude Paris, France
        position.longitude = 2.3522; // Longitude Paris, France
